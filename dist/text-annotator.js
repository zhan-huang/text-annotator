...
class TextAnnotator {
    ...
    eagerSearch(prefix, str, postfix, eagerSearchOptions = { containerId: '' }) {
    ...
    fuzzySearch(prefix, str, postfix, fuzzySearchOptions) {
        ...
        const sentenceBased = fuzzySearchOptions.sentenceBased === undefined ||
        ...
        if (tokenBased || prefix || postfix) {
            ...
            for (let i = 0; i < strIndexes.length; i++) {
                ...
                // f can be wider
                const f = text.substring(si - prefix.length, si) +
                    str +
